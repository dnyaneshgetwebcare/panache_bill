...
<div class="row" >
    ...
    <?php  $form = ActiveForm::begin(['enableClientValidation'=>false,'id'=>'display-form','layout' => 'horizontal', 'fieldConfig' => [
    ...
    <?= $form->field($model, 'deatils_type')->textInput(['maxlength' => true]) ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('.modal-header').html("<span style='color:#3a3636;font-size:14px'><b>Display</b><button type='button' class='close' data-dismiss='modal' aria-hidden='true'>×</button></span>");
 ...
 function submitTypeForm(){
      ...
      //alert($('#display-form').attr('action'));

    $.ajax({
    url:$('#display-form').attr('action'),
    type: 'post',
    dataType:'json',
    data:$("#display-form").serialize(),
    beforeSend: function(){
          $(".overlay").show();
        },
     complete: function(){
